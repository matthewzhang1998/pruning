...
class MLPModel(BaseModel):
    def __init__(self, params, input_size, output_size, seed, init,
        ...
        for ii in range(len(self.params.mlp_hidden_seq)):
            params = {'input_depth': input_size,
                      ...
                      'num_unitwise': self.params.mlp_hidden_seq[ii]
                      }
    ...
    def init_model(self, scope='mlp', use_dense=False):
        with tf.variable_scope(scope):
            for i in range(len(self.Network['Net'])):
                self.Network['Net'][i] = SparseDummyFullyConnected(
                    **self.Network['Params'][i]
                )
    ...
    def get_placeholders(self):
        placeholders = [net.placeholder for net in self.Network['Net']]
        return placeholders

    def run_dummy(self, input):
        Dummy = [None for _ in self.Tensor['Output']]
        Dummy[0] = input

        for i in range(len(self.Network['Net'])):
            input = Dummy[i+1] = self.Network['Net'][i](input)
        return Dummy